 ...
 */
 ...
 add_filter('the_content','r_the_content');
 add_action('wp_enqueue_scripts', 'r_load_scripts');
 add_action('wp_ajax_r_rate_recipe','r_rate_recipe');
 add_action("admin_init",'recipe_main_init');
 ...
 function r_the_content($the_content)
 {
 ...
 function r_load_scripts()
  {
 
	wp_register_script( 'jQuery', 'https://cdnjs.cloudflare.com/ajax/libs/jquery/1.9.1/jquery.min.js',1 );
...
function recipe_main_init()
{
	
add_filter('manage_recipe_posts_columns','r_add_new_recipe_function');
add_action('manage_recipe_posts_custom_column','manage_recipe_custom_columns',10,2);
	function r_add_new_recipe_function($column)
	{
	$new_columns = [];
	...
	function manage_recipe_custom_columns($columns , $post_id){
		switch ($columns) {
			case 'count':
				$recipe_data	=	get_post_meta($post_id, 'recipe_data', true);
